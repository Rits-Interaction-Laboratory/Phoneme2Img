...
from net import PromptEncoder,Encoder,Decoder,TextureNet,PhonemeVAE,ImageVAE
from dataset import ImageLang,Lang,ImageLang2
from lossfunc import style_loss_and_diffs,criterion_VAE,criterion_PCAVAE
from utils import set_seed,draw_pca_plot3,select_top_k_outputs,tensorFromSentence,select_random_output
...
def Train(epoch,nums,encoder,decoder,image_model,prompt_converter,phonemevae,imagevae, pipe, lang,imageono_dataloader,ono_dataloader,device):
    encoder.train()
    decoder.train()
    image_model.train()
    ...
    imagevae_optimizer=optim.Adam(imagevae.parameters(),lr=learning_rate)
    ...
    #-----------------------------------------------------------      画像復元  
            ...
            mu_img,log_var_img,z_img,ave_img,log_dev_img=imagevae(my_hidden)
...
#----------------------------------------------------------------　２つを近づける
            ...
            best_images,my_hidden2,log_var_img2=select_top_k_outputs(IMG_HIDDEN,mu_img,log_var_img,top_k=10)

            best_output_mean = best_outputs.mean(dim=0, keepdim=True)
            all_ono_features.append(best_output_mean.to(torch.float32).reshape(-1).detach().cpu().numpy())
            ...
            best_image_mean = best_images.mean(dim=0, keepdim=True)

            best_imagess=best_images.squeeze(1)
            
            best_images = torch.nn.functional.layer_norm(best_images, best_images.shape[-1:])
            norm_best_images = F.normalize(best_images, p=2, dim=-1)
            ...
            # loss_imgono,recon_loss,kl_loss,sq_error,log_var,var,nll_per,mse_loss=criterion_VAE(norm_my_hidden,mu,log_var,norm_best_outputs,log_var_p2)
            'loss_imgono→画像の特徴ベクトルmy_hiddenとVAEの出力best_outputs2の損失cos_loss+KLloss'
            loss_imgono,recon_loss,kl_loss,sq_error,log_var,var,nll_per,mse_loss=criterion_VAE(norm_best_images,mu,log_var,norm_best_outputs,log_var_p2)
    ...
    return train_ono_loss,train_ONO_loss,train_img_loss,train_imgono_loss,train_recon_hidden_loss,train_kl_loss,train_total_loss,train_recon_loss,train_sq_error_loss,train_log_var_loss,train_var_loss,train_nll_per_loss,train_mse_loss,encoder,decoder,image_model,prompt_converter,phonemevae , accu , all_decoded, ld_values, all_labels, batch_losses, all_ono_features, all_img_features, geneimg_labels, images_list, geneimg_features
...
def Valid(encoder, decoder, image_model, prompt_converter, phonemevae, pipe, lang, imageono_valid_dataloader, ono_valid_dataloader, device):
    ...
    with torch.no_grad():
        for __ in tqdm.tqdm(range(max_valid_iter)):
            ...
            try:
                ...
                loss_imgono, recon_loss, kl_loss, sq_error, log_var_l, var, nll_per, mse_loss = criterion_VAE(norm_my_hidden, mu, log_var, norm_best_outputs, log_var_p2)
    ...
    valid_ono_loss=valid_ono_loss/(max_valid_iter*(imageono_valid_dataloader.batch_size)) #ミニバッチ*イテレータの数で割ることで1データ当たりのLossの値を算出
    valid_ONO_loss=valid_ONO_loss/(max_valid_iter*(imageono_valid_dataloader.batch_size))
    valid_img_loss=valid_img_loss/(max_valid_iter*(imageono_valid_dataloader.batch_size))
    valid_recon_hidden_loss=valid_recon_hidden_loss/(max_valid_iter*(imageono_valid_dataloader.batch_size))
    valid_imgono_loss=valid_imgono_loss/(max_valid_iter*(imageono_valid_dataloader.batch_size))
    valid_kl_loss=valid_kl_loss/(max_valid_iter*(imageono_valid_dataloader.batch_size))
    valid_total_loss=valid_total_loss/(max_valid_iter*(imageono_valid_dataloader.batch_size))
    valid_recon_loss=valid_recon_loss/(max_valid_iter*(imageono_valid_dataloader.batch_size))
    valid_sq_error_loss=valid_sq_error_loss/(max_valid_iter*(imageono_valid_dataloader.batch_size))
    valid_log_var_loss=valid_log_var_loss/(max_valid_iter*(imageono_valid_dataloader.batch_size))
    valid_var_loss=valid_var_loss/(max_valid_iter*(imageono_valid_dataloader.batch_size))
    valid_nll_per_loss=valid_nll_per_loss/(max_valid_iter*(imageono_valid_dataloader.batch_size))
    valid_mse_loss=valid_mse_loss/(max_valid_iter*(imageono_valid_dataloader.batch_size))
    valid_accu=score/len(valid_ld_values)

    return valid_ono_loss, valid_ONO_loss, valid_img_loss, valid_imgono_loss, valid_recon_hidden_loss, valid_kl_loss, valid_total_loss, valid_recon_loss, valid_sq_error_loss, valid_log_var_loss, valid_var_loss, valid_nll_per_loss, valid_mse_loss ,valid_accu , valid_all_decoded , valid_ld_values, valid_img_features, valid_ono_features, valid_all_labels, valid_images_list
...
def main():
    ...
    imagevae=ImageVAE(num_samples=100).to(device)
    ...
    for epoch in range(epochs):
        ...
        #--------train
        train_ono,train_ONO,train_img,train_imgono,train_recon_hidden_loss,train_kl,train_total,train_recon_loss,train_sq_error_loss,train_log_var_loss,train_var_loss,train_nll_per_loss,train_mse_loss,encoder,decoder,image_model,prompt_converter,phonemevae ,accu, all_decoded , ld_values, all_labels, batch_losses, all_ono_features, all_img_features, geneimg_labels, images_list, geneimg_features = Train(epoch,nums,encoder,decoder,image_model,prompt_converter,phonemevae,imagevae,pipe,lang,imageono_train_dataloader,ono_train_dataloader,device)

        #--------validation
        # valid_ono_loss, valid_ONO_loss, valid_img_loss, valid_imgono_loss, valid_recon_hidden_loss, valid_kl_loss, valid_total_loss, valid_recon_loss, valid_sq_error_loss, valid_log_var_loss, valid_var_loss, valid_nll_per_loss, valid_mse_loss ,valid_accu, valid_all_decoded ,valid_ld_values ,valid_img_features, valid_ono_features, valid_all_labels, valid_images_list= Valid(encoder, decoder, image_model, prompt_converter, phonemevae, pipe, lang, imageono_valid_dataloader, ono_valid_dataloader, device)
        ...
        if epoch in target_epochs:
            ...
            draw_pca_plot3(epoch, nums, all_labels,       all_ono_features,   all_img_features,        pca_model=shared_pca, limits=common_limits, dir="VAE",mode="trainVAE")
            ...
            draw_pca_plot3(epoch, nums, all_labels,       all_ono_features,   target=None,        pca_model=shared_pca, limits=common_limits, dir="VAE",mode="onlyVAE")
        ...
        print( "[epoch num %d ] [ train_mse: %.6f]" % ( epoch+1, train_imgono) )
        ...
        #train
        writer.add_scalars('loss/ono2', {'train': train_ono}, epoch+1)
