...
入力が偶数なら偶数5枚、奇数なら奇数5枚を関連画像として学習
"""
...
from loss_5 import sekitori_loss_sum
from data_utils_5 import get_fixed_related_images_batch as sample_related_images_batch, fixed_label_images
from visualize_5 import (
...
import visualize_5
visualize_5.set_img_root("img_fixed")


def build_selected_labels(assignment, n):
    labels = np.zeros(n, dtype=int)
    for t, pred_idxs in enumerate(assignment):
...
for epoch in range(1, epochs + 1):
    ...
    with torch.no_grad():
        ...
        related_tr = sample_related_images_batch(torch.tensor([TRACK_DIGIT]).to(device))
    ...
    if epoch <= 10:
        ...
        generate_digit_variations(model, device, epoch, target_digit=TRACK_DIGIT, num_variants=10)
